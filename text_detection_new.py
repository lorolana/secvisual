...
def detect_text_uri(uri):
    ...
    #mark_list.append(1)
    return text_list, mark_list,bound_list


sample1=pd.read_excel("visuals_to_google 9-4-19.xlsx")
...
def checking_sample(sample,file_id,start,end): 
    ...
    for i in range(start,end):
        ...
        text_list,mark_list,bound_list= detect_text_uri(sample.loc[i,'URL'])
        df_text=pd.DataFrame(text_list)
        df_mark=pd.DataFrame(mark_list)
        df_bound=pd.DataFrame(bound_list)
        df2=pd.concat([df_text,df_mark,df_bound],axis=1)
        ...
        if len(df2)>0:            
            ...
            print("error!!!")
            df3=sample1.loc[i]
            error=pd.concat([error,df3], axis= 0, sort=False)
    #print(acc)
    #print(error)
    ...
    total_text["mark"]=0 
    total_text.to_csv("confcallvisuals"+file_id+"_text.csv")
    error.to_csv("error"+file_id+".csv")
    #acc_text.to_csv("text.csv")
    #print(text_list)
    #print(acc_text)    

checking_sample(sample1,"178400_179200",178400,179200)
#checking_sample(sample2,"2_2000",0,5)
